...
backup_files() {
	...
	FILE="$(date +%Y_%m_%d_%H.%M.%S).tar"
	${SUDO_CMD} mkdir -p "${BACKUP_DEST}"
	# do quick backup without compression
	if server_is_running; then
		echo -n "Starting running server backup to ${FILE}..."
		game_command save-off
		game_command save-all
		sleep 10
		${SUDO_CMD} tar -C "${SERVER_ROOT}" -cf "${BACKUP_DEST}/${FILE}" --exclude '*/logs/*' .
	...
	${SUDO_CMD} xz -z "${BACKUP_DEST}/${FILE}"
	echo -e "\e[39;1m done\e[0m\n"
